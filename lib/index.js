// @flow
/*      Test locally: 
- yarn dev && open http://localhost:3000/graphiql

Serve:
- git add . && git commit -m "hi" && git push
- ssh leckr@178.79.186.237
- pm2 stop 1 && cd week44/server && git stash && git pull && yarn build && yarn serve 
(or yarn dev if logging should be enabled)          */
...
Post.sync({ force: true }).then(() => {
  //nothing gets inserted at first
});

Sub.sync({ force: true });
//also subs get deleted
...
type Query {
  posts(channel: String!, pushtoken: String): [Post]
...
const resolvers = {
  Query: {
    posts: (_, { channel, pushtoken }) => {
      const where = pushtoken ? { channel, pushtoken } : { channel };
