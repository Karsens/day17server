...
const resolvers = {
  ...
  Mutation: {
    ...
    createPost: (
      ...
      Sub.findAll({ where: { channel } }).then(posts => {
        let cumres: string = "";

        posts.forEach(post => {
          if (post.pushtoken) {
            const res = sendPush(post.pushtoken, "Someone liked your post");
            cumres.concat(res);
          }
        });
    ...
    likePost: (_, { id }) => {
      ...
      Post.findOne({ where: { id } }).then(
        post =>
          post.pushtoken && sendPush(post.pushtoken, "Someone liked your post")
      );
